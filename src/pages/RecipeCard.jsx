...
export default function RecipeCard({ recipe, onNavigate }) {
  ...
  return (
    <div className="group transform transition-all duration-700 hover:scale-105" onClick={() => onNavigate('detail', recipe.id)}>
      <div className="relative bg-white/15 backdrop-blur-xl border border-white/25 rounded-2xl md:rounded-3xl overflow-hidden shadow-lg md:shadow-2xl shadow-blue-500/5 hover:shadow-blue-500/15 transition-all duration-500 cursor-pointer group-hover:bg-white/20">
        <div className="absolute top-4 right-4 z-20">
          <button onClick={(e) => { e.stopPropagation(); toggleFavorite(); }} className="p-2 bg-white/50 rounded-full">
        ...
        <div className="relative z-10 p-4 md:p-8">
          <span className={`text-xs font-semibold ${recipe.type === 'makanan' ? 'text-blue-700 bg-blue-100/90' : 'text-green-700 bg-green-100/90'} px-2 md:px-3 py-1 md:py-1.5 rounded-full`}>
